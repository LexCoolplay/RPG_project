...
class Hero(Creature):
    ...
    def update(self,screen, moving, rotaiton_turn):
        self.rotaition += rotaiton_turn
        self.rotate()
        if moving == 1:
            self.rect.x += self.vel * math.cos(self.rotaition * (math.pi / 180))
            self.rect.y -= self.vel * math.sin(self.rotaition * (math.pi / 180))
        if moving == -1:
            self.rect.x -= self.vel * math.cos(self.rotaition * (math.pi / 180))
            self.rect.y += self.vel * math.sin(self.rotaition * (math.pi / 180))
        self.draw(screen)
    def rotate(self):
        ...
        image_out = image.rotate(self.rotaition)
