...
def inventory_gui(screen):
    i_GUI = GUI()
    for j in range(2):
        for i in range(10):
            inv_button = InventoryButton('Inventory', exit_script())
            inv_button.set_position((16+i*32,16+j*32))
            inv_button.set_name(str(i)+' '+str(j))
            i_GUI.add(inv_button)
    return i_GUI
